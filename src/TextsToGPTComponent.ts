...
import { read_text_file_function } from "./ReadTextFilesComponent.js";
...
async function read_text_files_parse(ctx: any, payload: any) {
    ...
    async function texts_to_gpt_function(ctx, passed_documents_cdns, url, usage, prompt, temperature, model, overwrite) {
        ...
        if (passed_documents_are_valid) {
            ...
            passed_documents_cdns = await read_text_file_function(ctx, passed_documents_cdns);
        ...
        let read_documents_cdns = await read_text_file_function(ctx, url);

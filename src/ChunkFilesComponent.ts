...
let chunk_files_component = OAIBaseComponent
  ...
  .set('title', 'gpt I x P')
  .set('category', 'llm')
  .set('description', 'Run chat GPT [instructions]x[prompts] times')
  .setMethod('X-CUSTOM')
  .setMeta({
    source: {
      summary: 'Run chat GPT [instructions]x[prompts] times, allowing for example to run two prompts with the same instructions.',
      links: {
...
async function chunk_files_parse(ctx: any, payload: any) {

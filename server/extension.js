  ...
  functions: {
    _exec: async (payload, ctx) =>
    {
      ...
      if (payload.files)
      {
        ...
        const args = { temperature: temperature, top_p: top_p, allow_gpt3: allow_gpt3, allow_gpt4: allow_gpt4 };

        const cdn_response_array = [];
        for (let i = 0; i < files.length; i++)
        {
          const chunks_cdn = files[i];
          const llm_results = await loop_llm_on_chunks(ctx, chunks_cdn, instruction, llm_functions, args);
          const cdn_response = await save_json_to_cdn(ctx, llm_results);
  ...
  functions: {
    _exec: async (payload, ctx) =>
    {
      ...
      if (payload.files)
      {
        ...
        const embedder = new OmniOpenAIEmbeddings(ctx);
        const args = { embedder:embedder, nb_of_results: nb_of_results, allow_gpt3: allow_gpt3, allow_gpt4: allow_gpt4 };
        
        const cdn_response_array = [];
        for (let i = 0; i < files.length; i++)
        {
          const chunks_cdn = files[i];
          const query_answers = await query_chunks(ctx, chunks_cdn, query, args);
...
var components = [ChunkFilesComponent, LoopLLMComponent, QueryChunksComponent];
